...
describe('ValidationService', () => {
  ...
  describe('checkIssuer', () => {
    it('should return void when issuer is valid', async () => {
      ...
      await expect(
        service.checkIssuer(securityId.value, issuerId.value),
      ).resolves.toBeUndefined();

      expect(queryBusMock.execute).toHaveBeenCalledWith(
        new IsIssuerQuery(securityId.value, issuerId.value),
      );
    });
    ...
    it('should throw UnlistedKycIssuer when issuer is invalid', async () => {
      ...
      await expect(
        service.checkIssuer(securityId.value, issuerId.value),
      ).rejects.toThrow(UnlistedKycIssuer);
    });
  });
  ...
  describe('checkClearingActivated', () => {
    it('should return void when clearing is activated', async () => {
      ...
      await expect(
        service.checkClearingActivated(securityId.value),
      ).resolves.toBeUndefined();

      expect(queryBusMock.execute).toHaveBeenCalledWith(
        new IsClearingActivatedQuery(securityId.value),
      );
    });
    ...
    it('should throw ClearingDeactivated when clearing is not activated', async () => {
      ...
      await expect(
        service.checkClearingActivated(securityId.value),
      ).rejects.toThrow(ClearingDeactivated);
    });
  });
  ...
  describe('checkClearingDeactivated', () => {
    it('should return void when clearing is deactivated', async () => {
      ...
      await expect(
        service.checkClearingDeactivated(securityId.value),
      ).resolves.toBeUndefined();
    ...
    it('should throw ClearingActivated when clearing is activated', async () => {
      ...
      await expect(
        service.checkClearingDeactivated(securityId.value),
      ).rejects.toThrow(ClearingActivated);
    });
  });
  ...
  describe('checkRole', () => {
    it('should return void when role is valid via HasRoleQuery', async () => {
      ...
      await expect(
        service.checkRole(role.value, targetId.value, securityId.value),
      ).resolves.toBeUndefined();
  ...
  describe('checkPause', () => {
    it('should return void when pause via IsPausedQuery', async () => {
      ...
      await expect(
        service.checkPause(securityId.value),
      ).resolves.toBeUndefined();

      expect(queryBusMock.execute).toHaveBeenCalledTimes(1);
      expect(queryBusMock.execute).toHaveBeenCalledWith(
        new IsPausedQuery(securityId.value),
      );
    });
    ...
    it('should throw SecurityPaused when security is paused', async () => {
      ...
      await expect(service.checkPause(securityId.value)).rejects.toThrow(
        SecurityPaused,
      );
    });
  });
  ...
  describe('checkUnpause', () => {
    it('should return void when pause via IsPausedQuery', async () => {
      ...
      await expect(
        service.checkUnpause(securityId.value),
      ).resolves.toBeUndefined();
    ...
    it('should throw SecurityUnPaused when security is paused', async () => {
      ...
      await expect(service.checkUnpause(securityId.value)).rejects.toThrow(
        SecurityUnPaused,
      );
    });
  });
  ...
  describe('checkProtectedPartitions', () => {
    it('should work successfully', async () => {
      const security = new Security(
        SecurityPropsFixture.create({ arePartitionsProtected: true }),
      );

      await expect(
        service.checkProtectedPartitions(security),
      ).resolves.toBeUndefined();
    });

    it('should throw PartitionsUnProtected when security is unprotected', async () => {
      const security = new Security(
        SecurityPropsFixture.create({ arePartitionsProtected: false }),
      );

      await expect(service.checkProtectedPartitions(security)).rejects.toThrow(
  ...
  describe('checkUnprotectedPartitions', () => {
    it('should work successfully', async () => {
      ...
      await expect(
        service.checkUnprotectedPartitions(security),
      ).resolves.toBeUndefined();
    });
    ...
    it('should throw PartitionsProtected when security is protected', async () => {
      ...
      await expect(
        service.checkUnprotectedPartitions(security),
      ).rejects.toThrow(PartitionsProtected);
    });
  });
  ...
  describe('checkMaxSupply', () => {
    it('should work when max supply not exceeded with partitionId', async () => {
      ...
      queryBusMock.execute
        ...
        .mockResolvedValueOnce({
          payload: BigDecimal.fromString(
            (BigInt(amount.value.toString()) - BigInt(10000)).toString(),
          ),
        })
        .mockResolvedValueOnce({
          payload: BigDecimal.fromString(
            (
              BigInt(security.totalSupply!.value.toString()) + BigInt(10000)
            ).toString(),
          ),
        });

      await expect(
        service.checkMaxSupply(
          securityId.value,
      ...
      expect(queryBusMock.execute).toHaveBeenNthCalledWith(
        3,
        new GetMaxSupplyQuery(securityId.value),
      );
    });
    ...
    it('should work when max supply not exceeded without partitionId', async () => {
      ...
      queryBusMock.execute.mockResolvedValueOnce({
        payload: BigDecimal.fromString(
          (
            BigInt(security.totalSupply!.value.toString()) +
            BigInt(amount.value.toString())
          ).toString(),
        ),
      });
    ...
    it('should throw MaxSupplyByPartitionReached when partition limit exceeded', async () => {
      ...
      ).rejects.toThrow(MaxSupplyByPartitionReached);
    });
    ...
    it('should throw MaxSupplyReached when limit exceeded', async () => {
      ...
      await expect(
        service.checkMaxSupply(securityId.value, amount.value, security),
      ).rejects.toThrow(MaxSupplyReached);
    });
  });
  ...
  describe('checkDecimals', () => {
    ...
    it('should return void when decimals are within range', async () => {
      ...
      await expect(
        service.checkDecimals(security, amount.value.toString()),
      ).resolves.toBeUndefined();

      expect(CheckNums.hasMoreDecimals).toHaveBeenCalledWith(
        amount.value.toString(),
        security.decimals,
      );
    });
    ...
    it('should throw DecimalsOverRange when too many decimals', async () => {
      ...
      await expect(
        service.checkDecimals(security, amount.value.toString()),
      ).rejects.toThrow(DecimalsOverRange);
  ...
  describe('checkProtectedPartitionRole', () => {
    it('should return void when role check passes', async () => {
      const checkRoleSpy = jest
        .spyOn(service, 'checkRole')
        .mockResolvedValue(undefined);

      await expect(
        service.checkProtectedPartitionRole(
          partitionId.value,
          targetId.value,
          securityId.value,
        ),
      ).resolves.toBeUndefined();
      expect(checkRoleSpy).toHaveBeenCalledWith(
        expect.any(String),
        targetId.value,
        securityId.value,
      );
  ...
  describe('checkValidNounce', () => {
    ...
    it('should return void when nounce is valid', async () => {
      ...
      await expect(
        service.checkValidNounce(securityId.value, targetId.value, nonce),
      ).resolves.toBeUndefined();

      expect(queryBusMock.execute).toHaveBeenCalledWith(
        new GetNounceQuery(securityId.value, targetId.value),
      );
      expect(queryBusMock.execute).toHaveBeenCalledTimes(1);
    });
    ...
    it('should throw NounceAlreadyUsed when nounce is equal to nextNounce', async () => {
      ...
      await expect(
        service.checkValidNounce(securityId.value, targetId.value, nonce),
      ).rejects.toThrow(NounceAlreadyUsed);
    });
  ...
  describe('checkAccountInControlList', () => {
    it('should return void when adding and account is not in control list', async () => {
      ...
      await expect(
        service.checkAccountInControlList(
          securityId.value,
          targetId.value,
          true,
        ),
      ...
      expect(queryBusMock.execute).toHaveBeenCalledWith(
        new IsInControlListQuery(securityId.value, targetId.value),
      );
      expect(queryBusMock.execute).toHaveBeenCalledTimes(1);
    });
    ...
    it('should throw AccountAlreadyInControlList when adding and account exists', async () => {
      ...
      ).rejects.toThrow(AccountAlreadyInControlList);
    });
    ...
    it('should throw AccountNotInControlList when removing and account not found', async () => {
      ...
      ).rejects.toThrow(AccountNotInControlList);
    });
  });
  ...
  describe('checkAccountInIssuersList', () => {
    it('should return void when adding and account is not an issuer', async () => {
      ...
      await expect(
        service.checkAccountInIssuersList(
          securityId.value,
          targetId.value,
          true,
        ),
      ...
      expect(queryBusMock.execute).toHaveBeenCalledWith(
        new IsIssuerQuery(securityId.value, targetId.value),
      );
      expect(queryBusMock.execute).toHaveBeenCalledTimes(1);
    });
    ...
    it('should throw AccountIsAlreadyAnIssuer when adding and account exists', async () => {
      ...
      ).rejects.toThrow(AccountIsAlreadyAnIssuer);
    });
      ...
      queryBusMock.execute.mockResolvedValueOnce({
        payload: {
          amount: new BigDecimal(
            (BigInt(amount.value.toString()) + BigInt(150)).toString(),
          ),
        },
      });
      await expect(
        service.checkHoldBalance(
          securityId.value,
          partitionId.value,
          sourceId.value,
          holdId,
  ...
  describe('checkBalance', () => {
    it('should return void when balance is sufficient', async () => {
      queryBusMock.execute.mockResolvedValueOnce({
        payload: BigDecimal.fromString(
          (
            BigInt(amount.value.toString()) + BigInt(amount.value.toString())
          ).toString(),
        ),
      });

      await expect(
        service.checkBalance(securityId.value, targetId.value, amount.value),
      ).resolves.toBeUndefined();

      expect(queryBusMock.execute).toHaveBeenCalledWith(
        new BalanceOfQuery(securityId.value, targetId.value),
      );
    });
  ...
  describe('checkControlList', () => {
    it('should return true for whitelist when user is in list', async () => {
      ...
      expect(queryBusMock.execute).toHaveBeenNthCalledWith(
        1,
        new GetControlListTypeQuery(securityId.value),
      );
      expect(queryBusMock.execute).toHaveBeenNthCalledWith(
        2,
        new GetControlListCountQuery(securityId.value),
      );
      expect(queryBusMock.execute).toHaveBeenNthCalledWith(
        3,
        new GetControlListMembersQuery(securityId.value, 0, 2),
      );
    });
    ...
    it('should throw AccountNotInWhiteList when user not in whitelist', async () => {
      ...
      await expect(
        service.checkControlList(securityId.value, sourceId.value),
      ).rejects.toThrow(AccountNotInWhiteList);
    });
  });

  describe('checkMultiPartition', () => {
    const securityMulti = new Security(
      SecurityPropsFixture.create({ isMultiPartition: true }),
    );
    const securitySingle = new Security(
      SecurityPropsFixture.create({ isMultiPartition: false }),
    );

    it('should resolve void when no partitionId and single partition', async () => {
      await expect(
        service.checkMultiPartition(securitySingle),
      ).resolves.toBeUndefined();
    });

    it('should resolve void when partitionId matches default and single partition', async () => {
      await expect(
        service.checkMultiPartition(securitySingle, _PARTITION_ID_1),
      ).resolves.toBeUndefined();
    });

    it('should resolve void when partitionId provided and multi-partition', async () => {
      await expect(
        service.checkMultiPartition(securityMulti, partitionId.value),
      ).resolves.toBeUndefined();
    });

    it('should throw NotAllowedInMultiPartition when no partitionId and multi-partition', async () => {
      await expect(service.checkMultiPartition(securityMulti)).rejects.toThrow(
        NotAllowedInMultiPartition,
      );
    });
  ...
  describe('checkIssuable', () => {
    it('should resolve void when security is issuable', async () => {
      const security = new Security(
        SecurityPropsFixture.create({ isIssuable: true }),
      );
    ...
    it('should throw NotIssuable when security is not issuable', async () => {
      const security = new Security(
        SecurityPropsFixture.create({ isIssuable: false }),
      );

      await expect(service.checkIssuable(security)).rejects.toThrow(
        NotIssuable,
      );
    });
  });
  ...
  describe('checkValidVc', () => {
    ...
    it('should return issuer and credential when valid', async () => {
      const checkIssuerSpy = jest
        .spyOn(service, 'checkIssuer' as any)
        .mockResolvedValue(undefined);

      const result = await service.checkValidVc(
        signedCredential,
        targetEvmAddress,
        securityId.value,
      );
  ...
  describe('checkMaturityDate', () => {
    ...
    it('should resolve void when new maturity date is later', async () => {
      ...
      await expect(
        service.checkMaturityDate(
          securityId.value,
          (currentTime + 1000).toString(),
        ),
      ).resolves.toBeUndefined();
      expect(queryBusMock.execute).toHaveBeenCalledWith(
        new GetBondDetailsQuery(securityId.value),
      );
    });
