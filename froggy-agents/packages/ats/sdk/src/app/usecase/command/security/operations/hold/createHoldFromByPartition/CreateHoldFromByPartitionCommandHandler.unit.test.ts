  ...
  describe('execute', () => {
    describe('error cases', () => {
      it('throws CreateHoldFromByPartitionCommandError when command fails with uncaught error', async () => {
        ...
        await expect(resultPromise).rejects.toBeInstanceOf(
          CreateHoldFromByPartitionCommandError,
        );

        await expect(resultPromise).rejects.toMatchObject({
          message: expect.stringContaining(
            `An error occurred while creating hold from: ${errorMsg}`,
          ),
    ...
    describe('success cases', () => {
      it('should successfully create hold from source', async () => {
        ...
        transactionServiceMock
          .getHandler()
          .createHoldFromByPartition.mockResolvedValue({
            id: transactionId,
          });
        ...
        expect(
          transactionServiceMock.getHandler().createHoldFromByPartition,
        ).toHaveBeenCalledTimes(1);
