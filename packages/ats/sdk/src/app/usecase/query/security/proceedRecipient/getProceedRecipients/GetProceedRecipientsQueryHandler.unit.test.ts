...
describe('GetProceedRecipientsQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new GetProceedRecipientsQueryHandler(
      contractServiceMock,
  ...
  describe('execute', () => {
    it('throws GetProceedRecipientsQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetProceedRecipientsQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying GetProceedRecipients: ${errorMsg}`,
        ),
    ...
    it('should successfully get proceedRecipients data', async () => {
      ...
      queryAdapterServiceMock.getProceedRecipients.mockResolvedValueOnce([
        '0x',
      ]);
