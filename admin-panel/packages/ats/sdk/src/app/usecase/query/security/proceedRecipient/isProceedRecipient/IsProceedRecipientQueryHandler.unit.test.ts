...
describe('IsProceedRecipientQueryHandler', () => {
  ...
  beforeEach(() => {
    handler = new IsProceedRecipientQueryHandler(
      contractServiceMock,
  ...
  describe('execute', () => {
    it('throws IsProceedRecipientQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        IsProceedRecipientQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying isProceedRecipient: ${errorMsg}`,
        ),
    ...
    it('should successfully get data', async () => {
      ...
      expect(queryAdapterServiceMock.isProceedRecipient).toHaveBeenCalledWith(
        evmAddress,
        targetEvmAddress,
      );
    });
