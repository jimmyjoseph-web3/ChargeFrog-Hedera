...
} from './CanTransferByPartitionQuery';
  ...
  describe('execute', () => {
    it('throws CanTransferByPartitionQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        CanTransferByPartitionQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying can transfer by partition: ${errorMsg}`,
        ),
    ...
    it('should successfully check if can transfer by partition', async () => {
      ...
      expect(
        queryAdapterServiceMock.canTransferByPartition,
