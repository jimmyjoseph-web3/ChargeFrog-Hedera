  ...
  describe('execute', () => {
    it('throws GetClearingCreateHoldForByPartitionQueryError when query fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        GetClearingCreateHoldForByPartitionQueryError,
      );

      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while querying clearing create hold: ${errorMsg}`,
        ),
    ...
    it('should successfully get clearing create hold for by partition', async () => {
      ...
      accountServiceMock.getAccountInfo
        .mockResolvedValueOnce(accountEscrow)
        .mockResolvedValueOnce(accountHoldTo);

      queryAdapterServiceMock.getClearingCreateHoldForByPartition.mockResolvedValueOnce(
        clearingHold,
      );
      ...
      expect(
        queryAdapterServiceMock.getClearingCreateHoldForByPartition,
