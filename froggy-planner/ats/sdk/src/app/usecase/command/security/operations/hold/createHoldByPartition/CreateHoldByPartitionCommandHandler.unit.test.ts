...
} from './CreateHoldByPartitionCommand';
  ...
  describe('execute', () => {
      ...
      it('throws CreateHoldByPartitionCommandError when command fails with uncaught error', async () => {
        ...
        await expect(resultPromise).rejects.toMatchObject({
          message: expect.stringContaining(
            `An error occurred while creating hold: ${errorMsg}`,
          ),
    ...
    describe('success cases', () => {
      it('should successfully create hold', async () => {
        ...
        transactionServiceMock
          .getHandler()
          .createHoldByPartition.mockResolvedValue({
            id: transactionId,
          });
        ...
        expect(
          transactionServiceMock.getHandler().createHoldByPartition,
        ).toHaveBeenCalledTimes(1);
