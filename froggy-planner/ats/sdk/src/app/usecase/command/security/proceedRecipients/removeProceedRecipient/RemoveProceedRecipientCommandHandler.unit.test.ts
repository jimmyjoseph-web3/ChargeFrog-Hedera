  ...
  describe('execute', () => {
    it('throws RemoveProceedRecipientCommandError when command fails with uncaught error', async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(
        RemoveProceedRecipientCommandError,
      );
      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(
          `An error occurred while removing proceed recipient: ${errorMsg}`,
        ),
    ...
    it('should successfully add proceedRecipientiary', async () => {
      ...
      transactionServiceMock
        .getHandler()
        .removeProceedRecipient.mockResolvedValue({
          id: transactionId,
        });
      ...
      expect(
        transactionServiceMock.getHandler().removeProceedRecipient,
